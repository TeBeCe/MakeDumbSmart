//
//  NewFunctionDetailView.swift
//  MakeDumbSmart
//
//  Created by Martin Bachraty on 02/03/2021.
//  Copyright © 2021 Martin Bachratý. All rights reserved.
//

import SwiftUI

struct NewFunctionDetailView: View {
    @State var newFunction: NewFunction
        ...
        VStack{
            Form{
                ...
                Section(header: Text("Device Vendor"), footer: Text("Correct vendor will help other people setting similar devices")) {
                    TextField("e.g. Apple", text: $vendor)
                        .disableAutocorrection(true)
                ...
                Button(action:{
                    ...
                    let createdDevice = Device(id: 0, device_name: functionName,module_id: moduleIndex, device: deviceRealName, reseting: isResetable, glyph: selectedGlyph, is_active: false, type: deviceType, value: 0.0, max_level: Int(maxValue), room: roomIndex, processing: 0)
...
struct NewFunctionDetailView_Previews: PreviewProvider {
    static var previews: some View {
        NewFunctionDetailView(newFunction: NewFunction(id: 0, vendor: "Apple", deviceRealName: "AppleTV", functionName: "SwitchON/Off", rawData: "", rawDataLen: ""), nf: LoadJSONNewFunctionData(), dvcObj: LoadJSONData(), activeSheet: .constant(.first) )
    }
}
